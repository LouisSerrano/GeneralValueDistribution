...
class HordeModel:
    def __init__(self, state_dim, num_GVDs, num_actions = 4, num_quantiles = 51, kappa = 1, eta = 1e-3):
        """ HordeModel Class
        Args:
            state_dim (int): the dimension of the state space
            num_GVDs (int): the number of GVDs
    ...
    def create_model(self):
        ...
        tf.keras.layers.Dense(self.num_actions*self.num_quantiles*self.num_GVDs),
        tf.keras.layers.Reshape((self.num_GVDs, self.num_actions, self.num_quantiles))])
    ...
    @tf.function
    def train(self, states, target, actions, gvd_i):
        """
        We note N the number of quantiles, A the number of actions, B the batch_size and
         S the dimension of the state space.
        Computes the quantile huber loss for transitions from the corresponding gvd, and takes an optimizer step.
        Args:
            states (Numpy array): the vector representation of the last states (B, S)
            target (Numpy array) : the theta values of the target with shape (B, N)
            actions (Numpy array) : the last_actions with shape (B, A)
            gvd_i (int): the index of the GVD for which we want to take a training step
            """
        with tf.GradientTape() as tape:
            theta = self.gvd_model(states, gvd_i)
    ...
    @tf.function
    def avg_loss_train(self, states, targets, actions , gvd_i, pi):
        """
        We note N the number of quantiles, A the number of actions, B the batch_size and
         S the dimension of the state space.
    ...
    def gvd_model(self, states, index):
        """
        Args:
            states (Numpy array): vector representation of the last states
            index (int): the index of the GVD for which we want to compute the quantiles of the last states
        Returns :
            theta (Tensorflow): quantiles of the states for the GVD corresponding to index
        """

        g = self.gvd_index(index)
        theta = self.model(states)
        theta = tf.reduce_sum(tf.multiply(theta, g), 1)
        return theta
    ...
    def predict(self, state):
        ...
        Returns :
            theta (Tensorflow array): quantiles of the last state
        """
        if len(state.shape) == 1:
            state = state.reshape(1, self.state_dim)
        return self.model.predict(state)

    def gvd_index(self, index):
        """
        Args:
            index (int): the index of the GVD for which we want to compute the quantiles of the last states
        Returns :
            g (Tensorflow array): a slicing vector used to select the quantiles of the GVD corresponding to the index
        """
        g = np.zeros(self.num_GVDs)
        g[index] = 1
        g = tf.constant(g, tf.float32)
        g = tf.reshape(g, [1, self.num_GVDs, 1, 1])
        g = tf.tile(g, [1, 1, self.num_actions, self.num_quantiles])
        return g